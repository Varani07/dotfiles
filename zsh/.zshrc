...
steamfps() {
  local fps=${1:-60}
  DXVK_FRAME_RATE=$fps prime-run steam
}
watchsteamfps() {
  local fps=${1:-60}
  DXVK_FRAME_RATE=$fps MANGOHUD=1 prime-run steam
}
herofps() {
  local fps=${1:-60}
  DXVK_FRAME_RATE=$fps prime-run flatpak run com.heroicgameslauncher.hgl
}
watchherofps() {
  local fps=${1:-60}
...
alias condaon='eval "$(/home/varani/anaconda3/bin/conda shell.zsh hook)" && conda activate base'
